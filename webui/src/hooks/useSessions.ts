...
export function useSessions(): {
  ...
  toggleArchive: (key: string) => Promise<void>;
} {
  ...
  const togglePin = useCallback(async (key: string) => {
    const newState = await togglePinSession(tokenRef.current, key);
    setSessions((prev) =>
      prev.map((s) => (s.key === key ? { ...s, pinned: newState } : s)),
    );
  }, []);

  const toggleArchive = useCallback(async (key: string) => {
    const newState = await toggleArchiveSession(tokenRef.current, key);
    setSessions((prev) =>
      prev.map((s) => (s.key === key ? { ...s, archived: newState } : s)),
    );
  }, []);
