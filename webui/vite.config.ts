...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../pythinker/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
    ...
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the pythinker ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the pythinker gateway;
