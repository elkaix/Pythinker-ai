...
def main(target_pkg: str) -> int:
    ...
    out.append("    # padding for Homebrew's post-install pass to rewrite its")
    out.append("    # `install_name` to the absolute Cellar path. Rebuild dulwich")
    out.append("    # from sdist with `PURE=1`, which disables the C accelerator")
    out.append("    # entirely — there's no dylib left to relocate, and dulwich")
    out.append("    # transparently falls back to its pure-Python diff/pack/object")
    out.append("    # implementations (used here only by Pythinker's memory-commit")
    out.append("    # codepath, where perf is non-critical).")
    out.append('    ENV["PURE"] = "1"')
    out.append('    system libexec/"bin/pip", "install", "--force-reinstall",')
