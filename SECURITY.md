...
If you discover a security vulnerability in pythinker, please report it by:

1. **DO NOT** open a public GitHub issue
2. Open a [private security advisory](https://github.com/mohamed-elkholy95/Pythinker-ai/security/advisories/new) on GitHub, or email the maintainer at moelkholy1995@gmail.com
3. Include:
...
chmod 600 ~/.pythinker-ai/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.pythinker-ai/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.pythinker-ai/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run pythinker with a dedicated user account
...
- Keep authentication data in `~/.pythinker-ai/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade pythinker-ai
```
...
- Subscribe to security advisories for pythinker and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash pythinker
   sudo -u pythinker pythinker-ai gateway
   ```

3. **Set Proper Permissions**
   ```bash
   chmod 700 ~/.pythinker-ai
   chmod 600 ~/.pythinker-ai/config.json
   chmod 700 ~/.pythinker-ai/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.pythinker-ai/logs/pythinker.log
   ```
...
- **Chat history is stored locally** - protect the `~/.pythinker-ai` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.pythinker-ai/logs/pythinker.log
   ```
...
- GitHub Security Advisories: https://github.com/mohamed-elkholy95/Pythinker-ai/security/advisories
- Release Notes: https://github.com/mohamed-elkholy95/Pythinker-ai/releases
