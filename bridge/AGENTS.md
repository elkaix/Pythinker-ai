...
- `AUTH_DIR` default `~/.pythinker-ai/whatsapp-auth`; Baileys auth state lives here, sibling `media/` receives downloads.
- `PYTHINKER_AI_BRIDGE_SOURCE_DIR` dev-only; when set, Pythinker spawns the bridge from this path instead of `~/.pythinker-ai/bridge/`, so repo edits take effect on restart without a re-deploy. Requires a built `dist/index.js`.
