...
from pythinker.agent.tools import (
    ArraySchema,
    IntegerSchema,
    ObjectSchema,
    Schema,
...
def test_schema_validate_value_matches_tool_validate_params() -> None:
    """ObjectSchema.validate_value must agree with validate_json_schema_value and Tool.validate_params."""
...
def test_schema_classes_equivalent_to_sample_tool_parameters() -> None:
    """JSON Schema produced by Schema classes must match the hand-written dict so validation behavior stays consistent."""
...
def test_exec_extract_absolute_paths_captures_home_paths() -> None:
    cmd = "cat ~/.pythinker-ai/config.json > ~/out.txt"
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "~/.pythinker-ai/config.json" in paths
    assert "~/out.txt" in paths


def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.pythinker-ai/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.pythinker-ai/config.json" in paths
...
def test_exec_guard_blocks_home_path_outside_workspace(tmp_path) -> None:
    ...
    error = tool._guard_command("cat ~/.pythinker-ai/config.json", str(tmp_path))
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("pythinker.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import pythinker.agent.tools.shell as shell_mod
