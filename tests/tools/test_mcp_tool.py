...
import pythinker.agent.tools.mcp as mcp_mod
from pythinker.agent.tools.mcp import (
    MCPPromptWrapper,
    MCPResourceWrapper,
    MCPToolWrapper,
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("pythinker.agent.tools.mcp.logger.warning", _warning)
...
async def test_connect_mcp_servers_logs_stdio_pollution_hint(
    ...
    monkeypatch.setattr("pythinker.agent.tools.mcp.logger.error", _error)
...
@pytest.mark.asyncio
async def test_connect_mcp_servers_passes_configured_stdio_cwd(
    fake_mcp_runtime: dict[str, object | None],
    tmp_path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    registry = ToolRegistry()
    stacks = await connect_mcp_servers(
        {"test": MCPServerConfig(command="fake", cwd=str(tmp_path))},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()
