...
@pytest.mark.asyncio
async def test_runner_records_egress_policy_denial_as_error_event():
    """Egress policy denials must surface as status='error' in tool_events.

    Regression: ToolEgressGateway returned 'Policy denied: ...' without an
    'Error:' prefix, so AgentRunner._run_tool's `result.startswith("Error")`
    check left the event as status='ok'. fail_on_tool_error never tripped
    and policy rejections silently looked like successful work.
    """
    from pythinker.agent.runner import AgentRunner, AgentRunSpec
    ...
    provider.chat_with_retry = AsyncMock(return_value=LLMResponse(
        content="trying",
        tool_calls=[ToolCallRequest(id="call_1", name="forbidden", arguments={})],
    ))
    tools = ToolRegistry()  # forbidden never registered — denial path only
    pol = PolicyService(enabled=True)  # deny-default with empty allow-list
    gw = ToolEgressGateway(registry=tools, policy=pol)
    ...
    assert event["name"] == "forbidden"
    assert "Policy denied" in event["detail"]
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "pythinker.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "pythinker.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("pythinker.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
...
def test_snip_history_truncates_oversized_system_prompt_when_output_exceeds_context(monkeypatch):
    from pythinker.agent.runner import AgentRunner, AgentRunSpec
    ...
    def estimate_tokens(message):
        ...
        return len(content) if isinstance(content, str) else 4
    ...
    assert trimmed[-1] == {"role": "user", "content": "hello"}
    assert sum(estimate_tokens(msg) for msg in trimmed) <= 2_048
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("pythinker.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from pythinker.agent.runner import (
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from pythinker.agent.loop import AgentLoop
    from pythinker.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from pythinker.bus.events import InboundMessage
    from pythinker.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pythinker.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from pythinker.agent.runner import _MAX_LENGTH_RECOVERIES, AgentRunner, AgentRunSpec
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from pythinker.agent.runner import _BACKFILL_CONTENT, AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from pythinker.agent.loop import AgentLoop
    from pythinker.agent.runner import _BACKFILL_CONTENT
    from pythinker.bus.events import InboundMessage
    from pythinker.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from pythinker.agent.runner import _BACKFILL_CONTENT, AgentRunner, AgentRunSpec
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from pythinker.agent.runner import _MICROCOMPACT_KEEP_RECENT, AgentRunner
...
async def test_injection_cycles_capped_at_max():
    ...
    from pythinker.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from pythinker.bus.events import InboundMessage
...
async def test_followup_routed_to_pending_queue(tmp_path):
    ...
    from pythinker.agent.loop import UNIFIED_SESSION_KEY
    from pythinker.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from pythinker.agent.loop import AgentLoop
    from pythinker.agent.runner import _MAX_INJECTIONS_PER_TURN
    from pythinker.bus.events import InboundMessage
    from pythinker.bus.queue import MessageBus
...
async def test_drain_injections_on_empty_final_response():
    ...
    from pythinker.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner, AgentRunSpec
    from pythinker.bus.events import InboundMessage
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    monkeypatch.setattr("pythinker.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))
...
@pytest.mark.asyncio
async def test_after_execute_tools_hook_fires():
    """The after_execute_tools hook receives the per-tool result list once
    per iteration, after _execute_tools returns. Existing hooks (before_*,
    after_iteration) keep firing in the same order."""
    from pythinker.agent.hook import AgentHook, AgentHookContext
    from pythinker.agent.runner import AgentRunner, AgentRunSpec
    ...
    class _Recording(AgentHook):
        async def before_execute_tools(self, context: AgentHookContext) -> None:
            seen.append(("before_execute_tools", len(context.tool_calls or [])))

        async def after_execute_tools(self, context: AgentHookContext, results: list) -> None:
            seen.append(("after_execute_tools", len(results)))

        async def after_iteration(self, context: AgentHookContext) -> None:
            seen.append(("after_iteration", 0))
    ...
    # The three-event sequence must appear in this exact order for the tool iteration.
    # after_iteration also fires for the final (no-tool) iteration, but we only
    # assert the first three entries which cover the tool-call iteration.
    assert seen[:3] == [
        ("before_execute_tools", 1),
        ("after_execute_tools", 1),
        ("after_iteration", 0),
    ]


@pytest.mark.asyncio
async def test_process_direct_emits_tool_events(tmp_path):
    """When process_direct is called with on_tool_event, each tool that
    runs in the resulting agent run produces one ToolEvent with phase='end'
    on success or phase='error' on failure. args_preview / result_preview
    are capped at 80 chars."""
    from pythinker.agent.loop import AgentLoop, ToolEvent
    from pythinker.bus.queue import MessageBus

    events: list[ToolEvent] = []

    async def _record(ev: ToolEvent) -> None:
        events.append(ev)
    ...
    loop = AgentLoop(bus=MessageBus(), provider=provider, workspace=tmp_path, model="test-model")
    loop.tools.get_definitions = MagicMock(return_value=[])
    loop.tools.execute = AsyncMock(return_value="ok result")
    loop.consolidator.maybe_consolidate_by_tokens = AsyncMock(return_value=False)  # type: ignore[method-assign]

    result = await loop.process_direct("hello", on_tool_event=_record)

    assert result is not None
    assert len(events) == 1
    assert events[0].name == "list_dir"
    assert events[0].phase in {"end", "error"}
    assert len(events[0].args_preview) <= 80
    assert events[0].duration_ms is not None


@pytest.mark.asyncio
async def test_runner_llm_timeout_returns_synthetic_error(monkeypatch):
    """A hung provider call must surface as a synthesized timeout response,
    ...
    provider.chat_with_retry = hanging_chat
    tools = MagicMock()
    tools.get_definitions.return_value = []
    ...
    assert result.stop_reason == "error"
    assert result.final_content is not None
    assert "timed out" in result.final_content
    assert "0.1" in result.final_content


def test_agent_run_spec_accepts_egress_and_request_context():
...
async def test_run_tool_prefers_egress_over_registry_when_set():
    """When spec.egress is set, _run_tool routes through it instead of registry/tool.execute."""
    from pythinker.agent.runner import AgentRunner, AgentRunSpec
    from pythinker.providers.base import ToolCallRequest
    from pythinker.runtime.context import RequestContext
    ...
    class _Echo(Tool):
        ...
        async def execute(self, **kwargs):
            return f"registry-said:{kwargs['x']}"
    ...
    class _StubEgress:
        async def execute(self, ctx, name, params):
            captured["ctx"] = ctx
            captured["name"] = name
            captured["params"] = params
            return "egress-said-hi"
    ...
    provider = MagicMock()
    runner = AgentRunner(provider)
    tool_call = ToolCallRequest(id="t1", name="echo", arguments={"x": "hi"})
    result, event, exc = await runner._run_tool(spec, tool_call, external_lookup_counts={})
    assert result == "egress-said-hi"
...
async def test_run_tool_falls_back_to_registry_when_no_egress():
    """When spec.egress is None, _run_tool keeps using the existing path."""
    from pythinker.agent.runner import AgentRunner, AgentRunSpec
    from pythinker.providers.base import ToolCallRequest
    ...
    tool_call = ToolCallRequest(id="t1", name="echo", arguments={"x": "hi"})
    result, event, exc = await runner._run_tool(spec, tool_call, external_lookup_counts={})
    assert result == "registry-said:hi"
    assert event["status"] == "ok"
    assert exc is None


async def test_egress_sees_unknown_tool_attempts_too(tmp_path):
    """Unknown-tool calls must still flow through egress so policy/telemetry sees them."""
    import json
    ...
    try:
        reg = ToolRegistry()  # empty — every call is "unknown"
        pol = PolicyService(enabled=True, allowed_tools={"default": ["*"]})
        ...
        spec = AgentRunSpec(
            ...
            max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
            egress=gw,
            request_context=RequestContext.for_inbound(
                channel="cli", sender_id="u", chat_id="c", session_key="cli:c",
            ),
        )
        runner = AgentRunner(MagicMock())
        tool_call = ToolCallRequest(id="t1", name="ghost", arguments={})
        await runner._run_tool(spec, tool_call, external_lookup_counts={})
    finally:
        set_sink(None)

    rows = [json.loads(line) for line in log.read_text(encoding="utf-8").splitlines()]
    assert any(r["name"] == "tool_call" and r["attributes"].get("tool") == "ghost" for r in rows)


def test_agent_run_spec_rejects_egress_without_request_context():
    """Setting egress without request_context (or vice versa) must fail loudly."""
    import pytest

    from pythinker.agent.runner import AgentRunSpec

    class _StubEgress:
        async def execute(self, ctx, name, params):  # pragma: no cover
            return None

    # egress set, request_context unset -> ValueError
    ...
    # request_context set, egress unset -> ValueError
    from pythinker.runtime.context import RequestContext
