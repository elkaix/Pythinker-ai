...
from pythinker.command.builtin import cmd_new, register_builtin_commands
from pythinker.command.router import CommandContext, CommandRouter
from pythinker.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("pythinker.agent.loop.SessionManager"), \
         ...
         patch("pythinker.agent.loop.Dream"):
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from pythinker.agent.loop import UNIFIED_SESSION_KEY
        from pythinker.command.builtin import cmd_stop
