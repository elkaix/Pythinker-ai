...
from pythinker.config.schema import ChannelsConfig, Config
from pythinker.utils.restart import RestartNotice
...
def test_manager_enables_default_local_webui_when_config_is_missing():
    config = Config()

    with patch(
        "pythinker.channels.registry.discover_all",
        return_value={"websocket": WebSocketChannel},
    ):
        mgr = ChannelManager(config, MessageBus())

    websocket = mgr.channels["websocket"]
    assert websocket.config.enabled is True
    assert websocket.config.host == "127.0.0.1"
    assert websocket.config.websocket_requires_token is True


def test_manager_respects_explicit_disabled_webui_config():
...
def test_discover_plugins_loads_entry_points():
    from pythinker.channels.registry import discover_plugins
...
async def test_base_channel_passes_api_base_to_openai_transcription_provider():
    ...
    from pythinker.providers import transcription as transcription_mod
...
def test_openai_transcription_provider_honors_api_base_argument():
    from pythinker.providers.transcription import OpenAITranscriptionProvider
...
from pythinker.providers.transcription import (  # noqa: E402
    GroqTranscriptionProvider as _GroqProvider,
)
from pythinker.providers.transcription import (  # noqa: E402
    OpenAITranscriptionProvider as _OpenAIProvider,
)
...
async def test_transcription_provider_includes_language(tmp_path, provider_cls, language):
    ...
    with patch("pythinker.providers.transcription.httpx.AsyncClient", return_value=_stub_async_client(captured)):
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    ...
    monkeypatch.setattr("pythinker.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "pythinker.channels.registry.discover_all",
...
def test_builtin_channel_default_config():
    ...
    from pythinker.channels.telegram import TelegramChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("pythinker.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("pythinker.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("pythinker.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("pythinker.channels.manager.consume_restart_notice_from_env", return_value=notice):
