...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr(
        "pythinker.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("pythinker.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "pythinker.channels.matrix.asyncio.create_task", _fake_create_task
    )
