...
def test_openrouter_sets_default_attribution_headers() -> None:
    ...
    assert headers["X-OpenRouter-Title"] == "pythinker"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("pythinker.providers.openai_compat_provider.AsyncOpenAI") as mock_client:
        provider = OpenAICompatProvider(
            ...
            extra_headers={
                ...
                "X-OpenRouter-Title": "Pythinker Pro",
    ...
    headers = mock_client.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://pythinker.ai"
    assert headers["X-OpenRouter-Title"] == "Pythinker Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
...
def test_openai_compat_keeps_tool_calls_after_consecutive_assistant_messages() -> None:
    ...
    sanitized = provider._sanitize_messages([
        ...
        {"role": "assistant", "content": "yeah, 40k stars is close"},
        {
            "role": "assistant",
        ...
        {"role": "user", "content": "how many stars now?"},
    ])
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("PYTHINKER_AI_STREAM_IDLE_TIMEOUT_S", "0")
