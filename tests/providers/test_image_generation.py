"""Tests for image generation provider clients.

Covers the Ollama, OpenAI, and OpenAI Codex providers plus MiniMax mime
detection and the Gemini native API base. StepFun has a dedicated focused
suite in ``test_image_generation_stepfun.py``.
"""
...
async def test_codex_payload_and_response(monkeypatch) -> None:
    ...
    assert call["headers"]["originator"] == "pythinker"
    assert call["headers"]["User-Agent"] == "pythinker (python)"
    assert call["headers"]["X-Test"] == "1"
    body = call["json"]
