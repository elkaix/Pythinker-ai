...
`command/router.py` (98) — four-tier dispatch: priority (pre-lock), exact, prefix (longest-first), interceptors. `command/builtin.py` (347) — registers `/stop`, `/restart` (`os.execv` after 1 s), `/status`, `/new`, `/tasks`, `/task-output <task_id>`, `/task-stop <task_id>`, `/dream`, `/dream-log [SHA]`, `/dream-restore [SHA]`, `/help`.
