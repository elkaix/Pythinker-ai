...
Pythinker can act as a WebSocket server, allowing external clients (web apps, CLIs, scripts) to interact with the agent in real time via persistent connections.
...
## Quick Start
Generated configs enable the embedded WebUI locally by default on
`127.0.0.1:8765`. Keep the host on loopback for a local-only admin dashboard,
or use an SSH tunnel/VPN/reverse proxy with real authentication for remote
access.

### 1. Configure

For unauthenticated raw WebSocket clients, add to `config.json` under
`channels.websocket`:
...
async def main():
    async with websockets.connect("ws://127.0.0.1:8765/?client_id=alice") as ws:
        ...
        await ws.send(json.dumps({"content": "Hello pythinker!"}))
...
|-------|------|---------|-------------|
| `enabled` | bool | `true` | Enable the WebSocket server and embedded WebUI. |
| `host` | string | `"127.0.0.1"` | Bind address. The default is local-only. Use `"0.0.0.0"` only with TLS or an explicit insecure-LAN opt-in. |
...
1. Client sends `GET {tokenIssuePath}` with `Authorization: Bearer {tokenIssueSecret}` (or `X-Pythinker-Auth` header).
...
`chat_id` is a *capability*: anyone holding a valid WebSocket auth credential and the chat_id can attach to that conversation and see its output. This is safe for pythinker's local, single-user model. Multi-tenant deployments should namespace chat_ids per user (or introduce a per-tenant auth gate) — pythinker does not do this today.
...
- An HTTP file server serving the pythinker media directory
