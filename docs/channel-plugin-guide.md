...
Build a custom pythinker channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of pythinker (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.
...
```text
pythinker-channel-webhook/
├── pythinker_channel_webhook/
...
```python
# pythinker_channel_webhook/__init__.py
from pythinker_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# pythinker_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "pythinker-channel-webhook"
version = "0.1.0"
dependencies = ["pythinker-ai", "aiohttp"]

[project.entry-points."pythinker.channels"]
webhook = "pythinker_channel_webhook:WebhookChannel"
...
packages = ["pythinker_channel_webhook"]
```
...
Built-in channels use Pydantic config models (subclassing `Base` from `pythinker.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `pythinker.config.schema.Base`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `pythinker-channel-{name}` | `pythinker-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `pythinker_channel_{name}` | `pythinker_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/pythinker-channel-webhook
