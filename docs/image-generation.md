...
| `tools.imageGeneration.saveDir` | string | `"generated"` | Relative directory under pythinker's media directory for generated artifacts |
...
AIHubMix `gpt-image-2-free` is supported through AIHubMix's unified predictions API. Internally pythinker calls:
...
pythinker supports two Gemini image generation model families via Google's Generative Language API:
...
```text
A minimal app icon for pythinker: friendly robot head, rounded square, soft blue and white palette, clean vector style, no text
```
