...
rewritten in-place the next time `pythinker-ai onboard` refreshes the config.
...
```text
my(action="set", key="current_project", value="pythinker")
...
### "Make this a document I can share"

The separate `make_pdf` tool is enabled by default for long-form deliverables.
It renders a structured Markdown report into `workspace/reports/<slug>.pdf` and
returns the file path. The agent should then attach that path through the
`message` tool's `media` parameter instead of pasting the path into chat.

Install the optional writer dependency when you want real PDF output:

```bash
pip install 'pythinker-ai[reports]'
```

Supported input is intentionally small and predictable: `#` / `##` / `###`
headings, bullet lists, paragraphs, `**bold**`, `*italic*`, and inline code.
For richer reports with tables or images, ask the agent to write a custom script
in the workspace.
