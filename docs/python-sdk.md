...
Use pythinker as a library — no CLI, no gateway, just Python.
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying pythinker internals:

```python
from pythinker.agent import AgentHook, AgentHookContext
...
### `Pythinker.from_config(config_path=None, *, workspace=None)`
...
```python
from pythinker.agent import AgentHook
...
async def main() -> None:
    bot = Pythinker.from_config(workspace="/my/project")
