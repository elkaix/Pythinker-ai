...
pythinker can expose a minimal OpenAI-compatible endpoint for local integrations:
...
If `channel` points to a channel that is not enabled in your config, pythinker will queue the outbound event but no platform delivery will occur.
