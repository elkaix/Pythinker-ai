# Upstream upstream audit — 2026-05-14

Source fetched from `https://github.com/HKUDS/upstream.git` into local `upstream/*` remote refs. There is no shared git merge base with this repository, so updates must be ported surgically rather than merged or cherry-picked wholesale.
...
1. Cross-channel `message` tool persistence
   - Upstream branches: `upstream/fix/cross-channel-session-persist`.
...
- HKUDS hosted `upstream auth` provider.
