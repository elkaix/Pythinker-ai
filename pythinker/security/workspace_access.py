...
def workspace_sandbox_status(
    *,
    ...
    return WorkspaceSandboxStatus(
        ...
        provider="none",
        provider_label=_provider_label("none"),
        summary="Workspace restriction uses application-level guards.",
    )
...
def _env_system_provider(environ: dict[str, str] | None = None) -> str | None:
    ...
    explicit_provider = env.get("PYTHINKER_WORKSPACE_SANDBOX_PROVIDER")
    enforced = env.get("PYTHINKER_WORKSPACE_SANDBOX_ENFORCED")
    compatibility = env.get("PYTHINKER_SANDBOX_ENFORCED")
