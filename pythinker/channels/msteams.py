...
from pythinker.config.paths import get_workspace_path
...
CONFIG_FIELDS = {
    ...
    "required_secrets": ["channels.msteams.app_password"],
    "fields": ["enabled", "app_id", "app_password", "tenant_id", "host", "port", "path"],
...
class MSTeamsChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            logger.error("PyJWT not installed. Run: pip install pythinker-ai[msteams]")
            return
    ...
    async def _validate_inbound_auth(self, auth_header: str, activity: dict[str, Any]) -> None:
        ...
        if not MSTEAMS_AVAILABLE:
            raise RuntimeError("PyJWT not installed. Run: pip install pythinker-ai[msteams]")
