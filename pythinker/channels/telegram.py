...
from pythinker.channels.telegram_markdown import (  # noqa: F401  (re-exported for tests)
...
from pythinker.channels.telegram_media import (
    _get_extension,
    _get_media_type,
...
from pythinker.utils.helpers import split_message
...
CONFIG_FIELDS = {
    "label": "Telegram",
    "required_secrets": ["channels.telegram.bot_token"],
    "fields": ["enabled", "allow_from", "bot_token"],
...
class TelegramConfig(Base):
    ...
    # Brand-distinctive "AI just sparked on your message" reaction. Telegram's
    # Bot API restricts non-premium bots to a fixed allowed-reaction set
    # (see https://core.telegram.org/bots/api#reactiontypeemoji), so we pick
    # from that list — ⚡ both renders universally and signals "processing
    # started" better than the prior 👀 default. Other on-allowlist swaps:
    # ✍ (drafting reply), 🫡 (salute / acknowledged), 👨‍💻 (developer at work),
    # 🤓 (nerd-coder), 👾 (retro AI), 🤝 (deal received). Anything off-list
    # (e.g. 🐍, 🤖) is silently dropped by Telegram with REACTION_INVALID;
    # the failure is caught in ``_add_reaction`` so it never breaks delivery.
    react_emoji: str = "⚡"
    ...
    # Webhook mode (mode="webhook"); ignored under the default polling mode.
    webhook_url: str = ""
    ...
    def _strip_token(cls, v):
        # Bot tokens are pasted into the onboard wizard and frequently arrive
        # with a trailing newline; httpx rejects the outbound URL with
        # `InvalidURL("non-printable ASCII character")` and the gateway fails
        # to start with an opaque message.  Trim here so that never surfaces.
        return v.strip() if isinstance(v, str) else v

    @field_validator("webhook_path")
    @classmethod
    def _webhook_path_must_start_with_slash(cls, value: str) -> str:
    ...
    def _validate_webhook_config(self) -> "TelegramConfig":
...
class TelegramChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # Per-session ordered ingress: buffer + drain worker keyed by session.
    ...
    def _normalize_telegram_command(content: str) -> str:
        """Map Telegram-safe command aliases back to canonical pythinker commands."""
    ...
    # Expose the module-level helpers as class attributes so existing tests
    # that reach them via ``TelegramChannel`` / ``channel._get_extension``
    # keep working without edits. The implementations live in
    # ``pythinker/channels/telegram_media.py``.
    _get_extension = staticmethod(_get_extension)
    ...
    def _queue_key_for_message(message) -> str:
        """Return the session key used for ordered Telegram ingress."""
    ...
    async def _drain_ordered_updates(self, key: str) -> None:
        ...
        try:
            while self._running:
                ...
                for item in batch:
                    try:
                        ...
                        logger.warning(
                            "Telegram queued update handling failed for {}: {}", key, e
                        )
            ...
            logger.warning("Telegram ordered update worker failed for {}: {}", key, e)
        finally:
            if not self._inbound_buffers.get(key):
    ...
    async def _process_forward_command(
