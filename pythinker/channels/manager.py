...
from pythinker.utils.restart import (
...
def _default_webui_dist() -> Path | None:
    ...
    try:
        import pythinker.web as web_pkg  # type: ignore[import-not-found]
    except ImportError:
        return None
...
class ChannelManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._uptime_buckets: dict[str, deque[int]] = {}
        ...
        for name, cls in discover_enabled(enabled_names, _names=names).items():
            ...
            try:
                ...
                if cls.name == "websocket" and self._session_manager is not None:
                    ...
                    kwargs["agent_defaults"] = self.config.agents.defaults
                ...
                self._uptime_buckets[name] = deque([0] * 60, maxlen=60)
    ...
    def _resolve_transcription_key(self, provider: str) -> str:
        ...
        try:
            if provider == "openai":
                return self.config.providers.openai.api_key or ""
            return self.config.providers.groq.api_key or ""
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        if not hasattr(self, "_uptime_buckets"):
            self._uptime_buckets = {}
        for name, channel in self.channels.items():
            bucket = self._uptime_buckets.setdefault(name, deque([0] * 60, maxlen=60))
            bucket.append(1 if channel.is_running else 0)
        return {
            name: {
                "enabled": True,
                "running": channel.is_running,
                "uptime_buckets": list(self._uptime_buckets[name]),
            }
            for name, channel in self.channels.items()
        }
