...
from pythinker.utils.helpers import safe_filename, split_message
...
CONFIG_FIELDS = {
    "label": "Discord",
    "required_secrets": ["channels.discord.token"],
    "fields": ["enabled", "token", "allow_from", "allow_channels", "intents"],
...
if DISCORD_AVAILABLE:
    ...
    class DiscordBotClient(discord.Client):
        ...
        async def send_outbound(self, msg: OutboundMessage) -> None:
            """Send a pythinker outbound message using Discord transport rules."""
...
class DiscordChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not DISCORD_AVAILABLE:
            logger.error("discord.py not installed. Run: pip install pythinker-ai[discord]")
            return
