...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install pythinker-ai[matrix]"
    ) from e
...
CONFIG_FIELDS = {
    "label": "Matrix",
    "required_secrets": ["channels.matrix.access_token"],
    "fields": [
        "enabled",
        "homeserver",
        "user_id",
...
class MatrixChannel(BaseChannel):
    ...
    async def _handle_key_verification_event(self, event: "KeyVerificationEvent") -> None:
        ...
        if isinstance(event, KeyVerificationKey):
            ...
            if any(isinstance(r, ToDeviceError) for r in responses):
    ...
    async def _download_media_bytes(self, mxc_url: str, limit_bytes: int) -> bytes | None:
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
                async with session.get(media_url, params={"allow_remote": "true"}) as response:
                    if response.status >= 400:
                        logger.warning("Matrix download failed for {}: HTTP {}", mxc_url, response.status)
                        return None
            ...
            logger.warning("Matrix download failed for {}", mxc_url, exc_info=True)
            return None
