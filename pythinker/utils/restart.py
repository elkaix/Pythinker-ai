...
RESTART_NOTIFY_CHANNEL_ENV = "PYTHINKER_AI_RESTART_NOTIFY_CHANNEL"
RESTART_NOTIFY_CHAT_ID_ENV = "PYTHINKER_AI_RESTART_NOTIFY_CHAT_ID"
RESTART_STARTED_AT_ENV = "PYTHINKER_AI_RESTART_STARTED_AT"
RESTART_REASON_ENV = "PYTHINKER_AI_RESTART_REASON"
...
def format_restart_completed_message(notice: RestartNotice | str) -> str:
    """Build restart completion text and include elapsed time when available.

    Accepts either a ``RestartNotice`` (preferred) or a raw ``started_at`` string
    for backwards compatibility with the old call signature.
    """
    if isinstance(notice, RestartNotice):
        started_at_raw = notice.started_at_raw
        reason = notice.reason
    else:
        started_at_raw = notice
        reason = ""
    elapsed_suffix = ""
    if started_at_raw:
        try:
            ...
            pass
    if reason == "upgrade":
        return f"Upgraded and restarted{elapsed_suffix}."
    return f"Restart completed{elapsed_suffix}."


def set_restart_notice_to_env(*, channel: str, chat_id: str, reason: str = "") -> None:
    ...
    if reason:
        os.environ[RESTART_REASON_ENV] = reason
    else:
        os.environ.pop(RESTART_REASON_ENV, None)
...
def consume_restart_notice_from_env() -> RestartNotice | None:
    ...
    reason = os.environ.pop(RESTART_REASON_ENV, "").strip()
