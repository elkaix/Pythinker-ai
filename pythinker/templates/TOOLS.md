...
- Use `find_files`, `glob`, or `list_dir` to locate workspace paths before `read_file` when a path is uncertain.
...
- Commands have a configurable timeout (default 60s; config value `0` disables the hard limit), dangerous commands are blocked, and output is truncated.
...
- Use `cron` for scheduled reminders or recurring jobs; do not run `pythinker-ai cron` through `exec`.
