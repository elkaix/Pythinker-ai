...
def _locked_exchange_copilot_token(github_access_token: str) -> dict:
    """Exchange a GitHub OAuth token for a Copilot access token under flock.

    Synchronous: meant to be invoked via ``asyncio.to_thread`` so the file
    lock and the single HTTP GET both happen off the event loop.
    """
    timeout = httpx.Timeout(20.0, connect=20.0)
    with refresh_lock("github-copilot"):
        with httpx.Client(timeout=timeout, follow_redirects=True, trust_env=True) as client:
            response = client.get(
                DEFAULT_COPILOT_TOKEN_URL,
                headers=_copilot_headers(github_access_token),
            )
...
class GitHubCopilotProvider(OpenAICompatProvider):
    ...
    async def _get_copilot_access_token(self) -> str:
        ...
        # to refresh the same Copilot token. The flock is sync; the HTTP
        # exchange is a single GET, so we use sync httpx inside to_thread.
        payload = await asyncio.to_thread(_locked_exchange_copilot_token, github_token.access)
    ...
    async def chat_stream(
        ...
        on_tool_call_delta: Callable[[dict[str, object]], None] | None = None,
    ):
        await self._refresh_client_api_key()
