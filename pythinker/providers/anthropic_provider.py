    ...
    def _convert_user_content(content: Any) -> Any:
        ...
        for item in content:
            ...
            if not item.get("type"):
                ...
                # A tool that returned a bare dict lands here; coerce it to a
                # text block instead of emitting a block the API rejects with
                # "content.0.type: Field required".
                result.append({"type": "text", "text": str(item)})
                continue
            result.append(item)
    ...
    async def chat(
        ...
        try:
            ...
            if self._is_streaming_required_error(e):
                ...
                # 10-minute server-side timeout. Transparently retry via the
                # streaming path so callers don't need to know the
