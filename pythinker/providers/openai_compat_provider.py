...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://pythinker.ai",
    "X-OpenRouter-Title": "pythinker",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Pythinker attribution headers to OpenRouter requests by default."""
...
def _deep_merge(base: dict[str, Any], override: dict[str, Any]) -> dict[str, Any]:
    ...
    Nested dicts are merged key-by-key; all other types in ``override`` replace
    the corresponding key in ``base``.
    """
    merged = dict(base)
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_kwargs(
        ...
        # Merge user-configured extra_body last so it can override or extend
        # provider-specific defaults set above (thinking-style, kimi). Uses
        # recursive merge so nested dicts like
        # ``{"chat_template_kwargs": {"enable_thinking": false}}`` do not
    ...
    def _should_use_responses_api(
        ...
        api_type = getattr(self, "_api_type", "auto")
            ...
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    ...
                    if self._spec and self._spec.name == "github_copilot":
                        ...
                        # hide the real error.
                        raise
                    if getattr(self, "_api_type", "auto") == "responses":
                        raise
