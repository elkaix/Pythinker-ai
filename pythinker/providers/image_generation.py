...
from pythinker.utils.helpers import detect_image_mime

_OPENROUTER_ATTRIBUTION_HEADERS = {
    "HTTP-Referer": "https://github.com/mohamed-elkholy95/Pythinker-ai",
...
def image_gen_provider_names() -> tuple[str, ...]:
    """Return the registered image-gen provider names in registration order."""
    ...
    async def generate(
        ...
        *,
        ...
        try:
            ...
            from pythinker.providers.openai_codex_provider import DEFAULT_ORIGINATOR
        except ImportError:
            raise ImageGenerationError(self.missing_key_message)
...
class StepFunImageGenerationClient(ImageGenerationProvider):
    """Async client for StepFun image generation.
...
def _stepfun_size(
    ...
    StepFun expects ``WIDTHxHEIGHT`` (width x height, not the more common
    ``HxW`` order). Accepted sizes include ``1024x1024``, ``768x1360``,
    ``896x1184``, ``1360x768``, ``1184x896``.
    """
    if image_size and "x" in image_size.lower():
        return image_size
