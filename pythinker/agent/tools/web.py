...
from pythinker.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema
...
class WebSearchTool(Tool):
    ...
    def _effective_provider(self) -> str:
        ...
        creds = self.config.credentials_for(provider)
        if provider == "brave":
            api_key = creds.api_key or os.environ.get("BRAVE_API_KEY", "")
            return "brave" if api_key else "duckduckgo"
        if provider == "tavily":
            api_key = creds.api_key or os.environ.get("TAVILY_API_KEY", "")
            return "tavily" if api_key else "duckduckgo"
        if provider == "searxng":
            base_url = (creds.base_url or os.environ.get("SEARXNG_BASE_URL", "")).strip()
            return "searxng" if base_url else "duckduckgo"
        if provider == "jina":
            api_key = creds.api_key or os.environ.get("JINA_API_KEY", "")
            return "jina" if api_key else "duckduckgo"
        if provider == "kagi":
            api_key = creds.api_key or os.environ.get("KAGI_API_KEY", "")
    ...
    async def _search_brave(self, query: str, n: int) -> str:
        creds = self.config.credentials_for("brave")
        api_key = creds.api_key or os.environ.get("BRAVE_API_KEY", "")
        if not api_key:
    ...
    async def _search_tavily(self, query: str, n: int) -> str:
        creds = self.config.credentials_for("tavily")
        api_key = creds.api_key or os.environ.get("TAVILY_API_KEY", "")
        if not api_key:
    ...
    async def _search_searxng(self, query: str, n: int) -> str:
        creds = self.config.credentials_for("searxng")
        base_url = (creds.base_url or os.environ.get("SEARXNG_BASE_URL", "")).strip()
        if not base_url:
    ...
    async def _search_jina(self, query: str, n: int) -> str:
        creds = self.config.credentials_for("jina")
        api_key = creds.api_key or os.environ.get("JINA_API_KEY", "")
        if not api_key:
    ...
    async def _search_kagi(self, query: str, n: int) -> str:
        creds = self.config.credentials_for("kagi")
        api_key = creds.api_key or os.environ.get("KAGI_API_KEY", "")
        if not api_key:
