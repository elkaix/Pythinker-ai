...
class ReadFileTool(_FsTool):
    ...
    def _read_office_doc(self, fp: Path) -> str:
        from pythinker.utils.document import extract_text
...
class EditFileTool(_FsTool):
    ...
    def description(self) -> str:
        return (
            ...
            "If old_text matches multiple times, provide more context or set occurrence, "
            "line_hint, replace_all, and expected_replacements. Shows closest-match diagnostics on failure."
        )
    ...
    async def execute(
        ...
        try:
            ...
            if count > 1 and not replace_all:
                if occurrence is not None:
                    ...
                    return (
                        ...
                        "Provide more context, set occurrence to choose one match, or set replace_all=true."
                    )
            ...
            if expected_replacements is not None and len(selected) != expected_replacements:
                return (
                    f"Error: expected {expected_replacements} replacements but would make {len(selected)}."
                )
            new_content = content
