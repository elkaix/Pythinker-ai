...
Ports upstream's generate_image flow into Pythinker. The LLM is expected to
deliver finished artifacts by calling the message tool with the returned
paths in its ``media`` parameter; the tool itself does not push outbound
attachments. This keeps the delivery path uniform across streaming and
    ...
    def _resolve_reference_image(self, value: str) -> str:
        ...
        if not any(_is_relative_to(resolved, root) for root in allowed_roots):
            raise ImageGenerationError(
                "reference_images must be inside the workspace or pythinker media directory"
            )
