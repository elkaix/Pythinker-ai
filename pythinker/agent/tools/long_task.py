"""Sustained goal tools on the main agent.
...
``pythinker.session.goal_state.goal_state_runtime_lines``) so compaction cannot hide them.
...
finished successfully, or cancelled / superseded / redirected — in every case the
recap should match reality.

There is no sub-agent orchestrator and no special WebSocket stream; an active goal is
mirrored into Runtime Context and a lightweight ``_goal_state_sync`` frame is published
for WebSocket chats so the WebUI can reflect the current state.
"""
...
class _GoalToolsMixin:
    """Shared per-turn routing context (coroutine-safe) + Session lookup.

    Context is bound per turn by ``AgentLoop`` via :meth:`set_context`; ContextVars keep
    it task-local so concurrent sessions never read each other's routing state.
    """
    ...
    async def _publish_goal_state_ws(self, metadata: dict[str, Any]) -> None:
        ...
        bus = self._bus
        if bus is None or self._channel.get() != "websocket":
            return
        cid = (self._chat_id.get() or "").strip()
        if not cid:
            return
...
@tool_parameters(
    tool_parameters_schema(
        goal=StringSchema(
            "Sustained objective for this chat thread. Call this promptly once the user's intent "
            "is clear. The goal must be idempotent, self-contained, bounded, and explicit about "
            "done-ness; do not delay this tool call to over-plan, research, or decide execution "
            "details.",
            max_length=12_000,
        ),
...
class LongTaskTool(Tool, _GoalToolsMixin):
    ...
    def description(self) -> str:
        return (
            "Mark this thread as a sustained long-running task. Call this as soon as the user's "
            "intent is clear. Write a good idempotent goal, but do not delay the tool call with "
            "long planning, research, or execution-detail thinking. The active goal is mirrored in "
            "Runtime Context each turn. Use normal tools until done, then call complete_goal when "
            "the objective is satisfied, cancelled, or replaced. If a goal is already active, "
            "finish it or call complete_goal before registering another."
        )
...
class CompleteGoalTool(Tool, _GoalToolsMixin):
    ...
    def description(self) -> str:
        return (
            "End bookkeeping for the active sustained goal. Use when the objective is fully "
            "achieved and verified — recap what was delivered. Also call when the user cancels, "
            "redirects, or replaces the goal: recap must reflect what actually happened (not "
            "necessarily success). If no goal is active, the tool reports that and leaves metadata "
            "unchanged."
        )
