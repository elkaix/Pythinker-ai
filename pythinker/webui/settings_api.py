...
from pythinker.security.workspace_access import workspace_sandbox_status
from pythinker.webui.workspaces import (
...
# Backends that do not expose an OpenAI-compatible /models endpoint.
...
def _oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from pythinker.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def _model_catalog_kind(spec: Any) -> str:
    """Derive catalog kind from ProviderSpec attributes — no hardcoded lists."""
    if spec.is_local:
        return "local"
    if spec.is_direct:
        return "custom"
...
def provider_models_payload(query: QueryParams) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            **base_payload,
            "status": "error",
            "message": f"Model list request failed with HTTP {status_code}.",
        }
...
def settings_payload(
    *,
    ...
    selected_image_provider = next(
        (p for p in image_providers if p["name"] == image_config.provider),
        None,
    )
    model_presets = [
        {
    ...
    webui_allow_local = getattr(config.tools, "webui_allow_local_service_access", True)
    payload = {
        "agent": {
        ...
        "web": {
            ...
            "user_agent": getattr(config.tools.web, "user_agent", ""),
            "search": {
                "max_results": search_config.max_results,
                "timeout": search_config.timeout,
            },
            "fetch": {
                "use_jina_reader": getattr(
                    getattr(config.tools.web, "fetch", None), "use_jina_reader", False
                ),
            },
        },
        "image_generation": {
        ...
        "advanced": {
            ...
            "webui_allow_local_service_access": webui_allow_local,
            "allow_local_preview_access": webui_allow_local,
...
def update_agent_settings(query: QueryParams) -> dict[str, Any]:
    ...
    tool_hint_max_length = _query_first_alias(
        query, "tool_hint_max_length", "toolHintMaxLength",
    )
    ...
    if "api_type" in query:
        if spec.name == "openai":
            ...
            try:
                ...
                parsed_api_type = ProviderConfig(api_type=api_type).api_type  # type: ignore[call-arg]
            except Exception:
...
def logout_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "openai_codex":
        ...
        try:
            ...
            raise WebUISettingsError("GitHub Copilot OAuth support is unavailable", status=500) from None
        token_path = _storage().get_token_path()
    else:
...
def update_network_safety_settings(query: QueryParams) -> dict[str, Any]:
    ...
    if raw_allow is not None:
        _parse_bool(raw_allow, "webui_allow_local_service_access")  # validate only; field not in schema yet
    ...
    if raw_default_access_mode is not None:
        ...
        try:
            changed = write_webui_default_access_mode(default_access_mode)
...
def update_web_search_settings(query: QueryParams) -> dict[str, Any]:
    ...
    def set_fetch_value(attr: str, value: object) -> None:
        nonlocal changed
        fetch = getattr(web_config, "fetch", None)
        if fetch is not None and getattr(fetch, attr, None) != value:
            setattr(fetch, attr, value)
            changed = True
    ...
    if use_jina_reader is not None:
        ...
        fetch = getattr(web_config, "fetch", None)
        previous_jina_reader = getattr(fetch, "use_jina_reader", False) if fetch else False
        set_fetch_value("use_jina_reader", normalized in {"1", "true", "yes"})
        current_jina_reader = getattr(fetch, "use_jina_reader", False) if fetch else False
        if current_jina_reader != previous_jina_reader:
            restart_required = True
...
def update_image_generation_settings(query: QueryParams) -> dict[str, Any]:
    ...
    default_image_size = _query_first_alias(
        query, "default_image_size", "defaultImageSize",
    )
    ...
    max_images_per_turn = _query_first_alias(
        query, "max_images_per_turn", "maxImagesPerTurn",
    )
    ...
    if image_config.enabled:
        selected_provider = next(
            (p for p in _image_generation_provider_rows(config) if p["name"] == image_config.provider),
            None,
        )
