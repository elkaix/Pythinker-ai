...
Keep WebUI Settings route handlers here, not in ``channels/websocket/channel.py``.
...
_MCP_VALUES_HEADER = "X-Pythinker-MCP-Values"
...
class WebUISettingsRouter:
    ...
    async def _handle_settings_mcp_presets(
        ...
        if not self._authorized(request):
            return self._unauthorized()
        try:
            from pythinker.agent.tools.mcp import request_mcp_reload
            from pythinker.webui.mcp_presets_api import mcp_presets_settings_action
            ...
            return self._error_response(501, "MCP presets API is not available")
