...
UI-only metadata, scoped to the active Pythinker instance data directory (the
directory containing the current ``config.json``). It deliberately does not
modify agent sessions — clearing it removes WebUI overrides only.

State shape (schema v1):

* ``pinned_keys`` / ``archived_keys`` — session keys flagged in the sidebar.
* ``title_overrides`` — user-provided session titles, max 160 chars.
* ``tags_by_key`` — short labels attached to sessions, max 12 per session.
* ``collapsed_groups`` — group → collapsed bool.
* ``view`` — global sidebar preferences (density, previews, timestamps,
  archived visibility, sort key).

Writes are atomic (temp file + fsync + replace + dir fsync) and capped at
256 KiB so a runaway sidebar can't blow up. Reads normalize any older /
partial input to the current schema.
"""
...
def _clean_tags_by_key(value: Any) -> dict[str, list[str]]:
    ...
    for key, raw_tags in list(value.items())[:_MAX_MAP_ITEMS]:
        ...
        if cleaned_key is None:
            continue
        tags = _clean_string_list(raw_tags, max_len=_MAX_TAG_LEN)[:_MAX_TAGS_PER_KEY]
        if tags:
...
def write_webui_sidebar_state(raw: dict[str, Any]) -> dict[str, Any]:
    """Atomically write *raw* (normalized) to disk; return the persisted state."""
