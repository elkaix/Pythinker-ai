...
#
# Set PYTHINKER_INSTALL_QUIET_DEPRECATION=1 to silence the banner.

set -euo pipefail

if [[ "${PYTHINKER_INSTALL_QUIET_DEPRECATION:-0}" != "1" ]]; then
  cat >&2 <<'BANNER'
...
[pythinker] Silence this banner with PYTHINKER_INSTALL_QUIET_DEPRECATION=1.
